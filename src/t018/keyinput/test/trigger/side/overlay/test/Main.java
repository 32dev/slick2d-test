package t018.keyinput.test.trigger.side.overlay.test;
...
public class Main extends BasicGame {
    ...
    private final String[] LANE_KEYS = {
        ...
        // ------------------------------------
        // [추가] CAPITAL (Caps Lock) 및 ADD (NumPad +)
        // ------------------------------------
        "CAPITAL", "ADD"
    };
    ...
    private int totalLanes = LANE_KEYS.length; // 현재는 사용되지 않지만, 레인 개수 확인용
    ...
    public void render(GameContainer gc, Graphics g) throws SlickException {
        ...
        for (String key : LANE_KEYS) {
            ...
            // -----------------------------------------------------------------
            // **[수정]** 와이드 노트 (SPACE, RIGHT, CAPITAL, ADD) 처리
            // -----------------------------------------------------------------
            if (key.equals("SPACE") || key.equals("RIGHT") || key.equals("CAPITAL") || key.equals("ADD")) {
                ...
                if (key.equals("SPACE")) {
                    // Q(0) ~ E(2) 레인에 걸침 (3개 레인 너비)
                    startLaneIndex = 0;
                    endLaneIndex = 2;
                } else if (key.equals("RIGHT")) {
                    // NUMPAD7(3) ~ NUMPAD9(5) 레인에 걸침 (3개 레인 너비)
                    startLaneIndex = 3;
                    endLaneIndex = 5;
                } else if (key.equals("CAPITAL")) { 
                    // [추가] CAPS LOCK: Q(0) ~ E(2) 레인에 걸침, 색상: CYAN
                    startLaneIndex = 0;
                    endLaneIndex = 2;
                    noteColor = Color.cyan;
                } else if (key.equals("ADD")) {
                    // [추가] NUMPAD +: NUMPAD7(3) ~ NUMPAD9(5) 레인에 걸침, 색상: CYAN
                    startLaneIndex = 3;
                    endLaneIndex = 5;
                    noteColor = Color.cyan;
                }
                
                if (startLaneIndex == -1) continue; // 안전장치
            ...
            // 와이드 노트는 시작 레인 인덱스에서 이미 선이 그려집니다.
            if (laneIndex < 6) { // Q, W, E, NUMPAD7, NUMPAD8, NUMPAD9
        ...
        // 마지막 경계선 (전체 레인 끝)
        g.setColor(Color.darkGray);
        g.drawLine(6 * laneWidth, 0, 6 * laneWidth, gc.getHeight());
